...
def get_session(h, wd):
    ...
    if  9<=h<17: return {"name":"Evropa","mult":1.0,"note":"Standardní"}
    if 15<=h<23: return {"name":"Amerika","mult":1.3,"note":"Velké pohyby"}
...
def classify_trade(r1m, r3m, r5m, r15, r1h) -> dict:
    """
    Určí nejvhodnější typ obchodu na základě stavu indikátorů na všech TF.
    Principy:
      Scalp:       1M extrémně přeprodaný, 3M neutrální/mírně low → rychlý odraz
      Micro-swing: 3M+5M přeprodané, 15M má prostor → delší pohyb
      Swing:       15M přeprodaný, 1H má prostor → hodinový pohyb
    """
    scores = {}
    ...
    if r1m["rsi"] <= 30:  sc += 3
    elif r1m["rsi"] <= 40: sc += 1
    if r1m["mh"] > r1m.get("mh_prev", r1m["mh"]): sc += 2  # MACD hist se otáčí
    # Negativní: pokud 3M/5M také extrémně low → micro-swing, ne scalp
    if r3m["sk"] <= 20: sc -= 2
    if r5m["sk"] <= 20: sc -= 2
    if r15["rsi"] < 38: sc -= 3  # 15M nízko → swing, ne scalp
    scores["scalp"] = max(0, sc)

    # MICRO-SWING: 3M+5M cyklus
    ms = 0
    if r3m["sk"] <= 25:  ms += 3
    elif r3m["sk"] <= 40: ms += 1
    if r5m["sk"] <= 30:  ms += 3
    ...
    if r5m["rsi"] < 48:  ms += 2
    if r3m["mh"] < 0:    ms += 2  # 3M MACD záporný = prostor
    if r15["rsi"] < 50 and r15["rsi"] > 35: ms += 2  # 15M má prostor ale není swing
    if r5m.get("vol_ratio",1) > 1.3: ms += 1  # volume potvrzení
    # Negativní: 5M overbought = konec micro-swing
    if r5m["sk"] > 70: ms -= 3
    if r15["rsi"] < 35: ms -= 2  # příliš nízko = swing
    scores["micro_swing"] = max(0, ms)

    # SWING: 15M cyklus
    sw = 0
    if r15["rsi"] < 40:  sw += 4
    elif r15["rsi"] < 48: sw += 2
    if r15["sk"] <= 30:  sw += 3
    if r5m["mh"] < 0:    sw += 2  # 5M MACD záporný
    if r15["mh"] < 0:    sw += 3  # 15M MACD záporný = silný swing potenciál
    if r1h["rsi"] < 55 and r1h["rsi"] > 30: sw += 2
    if r1h["e9"] >= r1h["e21"] * 0.995: sw += 1  # 1H bull trend
    # Negativní: 15M SK overbought
    if r15["sk"] > 60: sw -= 2
    ...
    if r1h["rsi"] < 40:  sp += 4
    if r1h["sk"] <= 25:  sp += 3
    if r1h.get("mh",0) < 0: sp += 3
    if r1h.get("e9",0) < r1h.get("e21",0) * 0.998: sp += 2  # 1H downtrend = deep swing
    scores["swing_plus"] = max(0, sp)

    best = max(scores, key=scores.get)
    best_score = scores[best]
    confidence = "vysoká" if best_score >= 7 else "střední" if best_score >= 4 else "nízká"

    tt = TRADE_TYPES[best]
    return {
        "type": best, "scores": scores,
        "confidence": confidence, "best_score": best_score,
        **tt,
    }
...
def predict_timing(df1m, rsi_cur, trade_type):
    ...
    if not cycles:
        ...
        return {"eta":base,"lo":base//2,"hi":base*2,
...
def analyze(equity=10000, risk_pct=1.0, do_validate=True):
    ...
    for lbl,val,note in [("WO",wo,"weekly open"),("PDH",pdh,"včera max"),
        ...
        if val==0: continue
        d=cur-val; m="◄◄" if abs(d)<50 else "◄" if abs(d)<120 else ""
        print(f"  {lbl:<12} {val:>10,.0f}  {d:>+8.0f}  {note} {m}")
